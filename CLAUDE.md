...
**Next action:** the bee-factory integration run. `test/integration/bee.test.ts` is written and skips itself unless `DAPPDATA_BEE_URL` and `DAPPDATA_STAMP` are set; it covers both transports, C5 on the wire and the blob path. It has not run: port 1633 on this machine belongs to Swarm Desktop's **mainnet** node, which is off limits, and bee-factory's queen wants that port. Ask Peter to stop Swarm Desktop, then `npx @ethersphere/bee-factory start`, buy a batch on it, and run `pnpm --filter dappdata test:integration`. After that: promote the fetch transport to the default (D18), then Phase 2 funding (`funding/`, `stamper/`, D3, D19, D23). Still open and not needed before Phase 1 code: D6, D7 (Phase 4), D14 (API shape), D19, D23 (Phase 2), D25 items 1 and 3 (Phase 1), D24 (awaiting swarm-id). The Sepolia writer's batch `98dfbb97…` expired and was pruned; the wallet holds 0.209 sETH and 0.066 sBZZ, so a fresh batch needs buying when Sepolia is next used (Phase 1 runs on bee-factory).
