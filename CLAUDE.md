...
**Next action:** the Phase 2 gate needs the Sepolia run — user-pays and sponsor-pays end to end through one code path, and a second device restoring stamper state without reusing a slot. The writer wallet holds 0.209 sETH and 0.066 sBZZ, enough for a shallow batch; `spikes/s3/src/sbzz.mjs` swaps for more. Still to write in Phase 2: `entropy.passkey()` over WebAuthn PRF (D21, D25). Note for any session: bee-factory holds port 1633 while it runs, so Swarm Desktop stays off until `npx @ethersphere/bee-factory stop`.
