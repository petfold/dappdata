...
**Next action:** `transport/` and `feed/`, then `slot` and `DappData.connect`, then the README example running against bee-factory (Phase 1 gate, C1). Still open and not needed before Phase 1 code: D6, D7 (Phase 4), D14 (API shape), D19, D23 (Phase 2), D25 items 1 and 3 (Phase 1), D24 (awaiting swarm-id). The Sepolia writer's batch `98dfbb97…` expired and was pruned; the wallet holds 0.209 sETH and 0.066 sBZZ, so a fresh batch needs buying when Sepolia is next used (Phase 1 runs on bee-factory).
