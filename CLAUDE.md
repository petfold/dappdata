...
**Next action:** push `main`, post the gate comment on IDEA-190 (Peter approves the text first), then start Phase 1 in `packages/dappdata`. Still open and not needed before Phase 1 code: D6, D7 (Phase 4), D14 (API shape), D19, D23 (Phase 2), D25 items 1 and 3 (Phase 1), D24 (awaiting swarm-id). The Sepolia writer's batch `98dfbb97…` expired and was pruned; the wallet holds 0.209 sETH and 0.066 sBZZ, so a fresh batch needs buying when Sepolia is next used (Phase 1 runs on bee-factory).
