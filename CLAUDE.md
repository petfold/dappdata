...
**D27 landed, 2026-09-22.** A blob is the sealed envelope split client-side (`transport/blob.ts`, core-sdk `ChunkSplitter`), every chunk stamped and posted to `/chunks`, root last; the feed carries the sealed 32-byte root; reads join through `/bytes` and open the inner frame; pre-D27 64-byte references still read. D9 amended: one encryption, ours. Sponsor-pays now covers values of any size, and `quote()` takes `bytesPerWrite` to size a batch for blobs (D23).

**Phase 2 gate signed off 2026-09-22.** Peter closed D19, D21, D23 and D27. Phase 3 (the reference dapp, `apps/demo`) may start; D26 (repo home, npm name) waits on tech leadership; D6 and D28 are Phase 4 design items with their numbers recorded.

**Gnosis mainnet run, 2026-09-22** (`spikes/phase2/RESULTS.md`, `src/mainnet.mjs`; Peter approved the spend and lifted the Swarm Desktop node's off-limits status for it). Funding rehearsal done on the real contracts: two depth-17 batches at 0.0249 xBZZ, `batchTTL` matches the contract on Gnosis, non-owner topUp works; self-stamping store, blob, network restore and the D4 full-bucket refusal all as designed; 13 stamps, no collision. Visibility on mainnet: first poll, every time. Our own latency is the finding: 11–13 s per write from probe misses and checkpoint churn at depth 17, a Phase 3 item in PLAN. Throwaway Gnosis payer key at `~/.dappdata-gnosis-payer.key` (0x1c58…C5e0), whitelisted in Swarm Desktop's Bee config for withdrawals, holds about 0.42 xBZZ. Swarm Desktop's node on `:1633` may be used for reads and client-stamped writes with Peter's say-so; it holds none of our batches.

**Next action:** Phase 3, `apps/demo`: sign-in, two slots, sponsor switch, fresh-profile restore, instrumentation; first-run path on a user-run node with `deferred` and `pin`, empty state shown at once; wallet matrix rows for Coinbase Wallet, WalletConnect mobile, Ledger, Trezor, an embedded wallet and a 7702 account. A third upstream issue is worth filing on Bee for the `batchTTL` arithmetic, drafted only when Peter says so. Note for any session: bee-factory holds port 1633 while it runs, so Swarm Desktop stays off until `npx @ethersphere/bee-factory stop`.
