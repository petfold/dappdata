...
**Next action:** the Phase 2 gate needs the Sepolia run — user-pays and sponsor-pays end to end through one code path, and a second device restoring stamper state without reusing a slot. The payer key `0x6f49…f3Bc` holds 0.5252 sBZZ and 0.0215 sETH after the 2026-09-21 swap, which is several depth-17 batches; `spikes/s3/src/sbzz.mjs` swaps for more (0.01 sETH ≈ 0.051 sBZZ, no faucet exists). Still to write in Phase 2: `entropy.passkey()` over WebAuthn PRF (D21, D25). Note for any session: bee-factory holds port 1633 while it runs, so Swarm Desktop stays off until `npx @ethersphere/bee-factory stop`.
