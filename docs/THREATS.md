...
**Mitigation.** The writer serves its own last write from memory, so a dapp never loses sight of what it just wrote. `expectIndex` still catches every conflict outside the window, and `merge` resolves it. Inside the window nothing client-side can help: closing it needs the per-device slot ranges of D19, or an application-level lock. The Phase 3 demo writes from one device at a time and says so. **Status:** open until D19; documented in D6.
**Not reproduced on a Gnosis mainnet light node (IDEA-198 study, 2026-09-22).** Reading back through the uploading node, all 13 first reads after a write answered 200; the first read took about 270 ms against 12 ms for later reads, never a 500. So the error window is a bee-factory observation so far, and the effect on mainnet is a slow first read. The mitigation stays: the study measured about 1.3 s until a public gateway saw a fresh update, so the two-writer race is about a second on either reading. A second node reading inside the first second was not tested on either rung.
